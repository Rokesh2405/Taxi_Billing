...
function gettrip($a,$b) {
    global $db;
    $get1 = $db->prepare("SELECT * FROM `trip` WHERE `id`=?");
...
function addtrip($billtype,$fromcity,$tocity,$driver_name,$driver_mobileno,$vehicleno,$permit_charge,$waiting_charge,$gstpercent,$customer_name,$order_method,$date,$gsttype,$bill_no,$driver,$trip_package,$trip_type,$vehicle_model,$total_km,$total_km_charge,$toll_charge,$driver_allowance,$total_charge,$per_km_charge,$pickup_date,$drop_date,$pickup_time,$drop_time,$start_km,$end_km,$sub_total,$gst_amt,$gstpercentval,$grand_total,$getid)
{
   global $db;
    if ($getid == '') {
        
          $resa = $db->prepare("INSERT INTO `trip` (`billtype`,`fromcity`,`tocity`,`driver_name`,`driver_mobileno`,`vehicleno`,`permit_charge`,`waiting_charge`,`gstpercent`, `customer_name`, `order_method`, `date`, `gsttype`, `bill_no`, `driver`, `trip_package`,`trip_type`,`vehicle_model`,`total_km`,`total_km_charge`,`toll_charge`,`driver_allowance`,`total_charge`,`per_km_charge`,`pickup_date`,`drop_date`,`pickup_time`,`drop_time`,`start_km`,`end_km`,`sub_total`,`gst_amt`,`gstpercentval`,`grand_total`) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $resa->execute(array($billtype,$fromcity,$tocity,$driver_name,$driver_mobileno,$vehicleno,$permit_charge,$waiting_charge,$gstpercent,$customer_name,$order_method,$date,$gsttype,$bill_no,$driver,$trip_package,$trip_type,$vehicle_model,$total_km,$total_km_charge,$toll_charge,$driver_allowance,$total_charge,$per_km_charge,$pickup_date,$drop_date,$pickup_time,$drop_time,$start_km,$end_km,$sub_total,$gst_amt,$gstpercentval,$grand_total));
  $l_insert = $db->lastinsertid();
  $url="../MPDF/trip_invoice.php?id=".$l_insert;   
  ...
  echo "<script>window.open('".$url."', '_blank');</script>";
            ...
            $resa = $db->prepare("UPDATE `trip` SET `billtype`=?,`fromcity`=?,`tocity`=?,`driver_name`=?,`driver_mobileno`=?,`vehicleno`=?,`permit_charge`=?,`waiting_charge`=?,`gstpercent`=?, `customer_name`=?, `order_method`=?, `date`=?, `gsttype`=?, `bill_no`=?, `driver`=?, `trip_package`=?,`trip_type`=?,`vehicle_model`=?,`total_km`=?,`total_km_charge`=?,`toll_charge`=?,`driver_allowance`=?,`total_charge`=?,`per_km_charge`=?,`pickup_date`=?,`drop_date`=?,`pickup_time`=?,`drop_time`=?,`start_km`=?,`end_km`=?,`sub_total`=?,`gst_amt`=?,`gstpercentval`=?,`grand_total`=? WHERE `id`=?");
            $resa->execute(array($billtype,$fromcity,$tocity,$driver_name,$driver_mobileno,$vehicleno,$permit_charge,$waiting_charge,$gstpercent,$customer_name,$order_method,$date,$gsttype,$bill_no,$driver,$trip_package,$trip_type,$vehicle_model,$total_km,$total_km_charge,$toll_charge,$driver_allowance,$total_charge,$per_km_charge,$pickup_date,$drop_date,$pickup_time,$drop_time,$start_km,$end_km,$sub_total,$gst_amt,$gstpercentval,$grand_total, $getid));
...
function deltrip($a) {
    ...
    foreach ($b as $c) {
        global $db;
          ...
          $get1 = $db->prepare("DELETE FROM `trip` WHERE `id` =? ");
...
function getpackage($a,$b) {
    global $db;
    $get1 = $db->prepare("SELECT * FROM `package` WHERE `id`=?");
    $get1->execute(array($b));
    $get = $get1->fetch(PDO::FETCH_ASSOC);
    $res = $get[$a];
    return $res;
...
function addpackage($package,$fromcity,$tocity,$package_price,$gst,$driver_beta,$toll_charge,$status,$getid)
{
   global $db;
    if ($getid == '') {
        $link2 = FETCH_all("SELECT `id` FROM `package` WHERE `package`=?", $package);
        if ($link2['id'] == '') {
            $resa = $db->prepare("INSERT INTO `package` (`package`, `fromcity`, `tocity`, `package_price`, `gst`, `driver_beta`, `toll_charge`, `status`) VALUES(?,?,?,?,?,?,?,?)");
            $resa->execute(array($package,$fromcity,$tocity,$package_price,$gst,$driver_beta,$toll_charge,$status));
 

  $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><h4><i class="icon fa fa-check"></i>Successfully Inserted</h4></div>';
        } else {
            $res = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><h4><i class="icon fa fa-close"></i>This Package is already exists.</h4></div>';
        }
    } else {
        $link1 = FETCH_all("SELECT `id` FROM `package` WHERE `package`=? AND `id`!=?", $package, $getid);
        if ($link1['id'] == '') {
            $resa = $db->prepare("UPDATE `package` SET `package`=?, `fromcity`=?, `tocity`=?, `package_price`=?, `gst`=?, `driver_beta`=?, `toll_charge`=?, `status`=? WHERE `id`=?");
            $resa->execute(array($package,$fromcity,$tocity,$package_price,$gst,$driver_beta,$toll_charge,$status, $getid));
...
function delpackage($a) {
    ...
    foreach ($b as $c) {
        global $db;
          ...
          $get1 = $db->prepare("DELETE FROM `package` WHERE `id` =? ");
         $get1->execute(array(trim($c)));
    }
...
function getcity($a,$b) {
    global $db;
    $get1 = $db->prepare("SELECT * FROM `city` WHERE `id`=?");
    $get1->execute(array($b));
    $get = $get1->fetch(PDO::FETCH_ASSOC);
    $res = $get[$a];
    return $res;
...
function addcity($city,$status,$getid)
{
   global $db;
    if ($getid == '') {
        $link2 = FETCH_all("SELECT `id` FROM `city` WHERE `city`=?", $city);
        if ($link2['id'] == '') {
            $resa = $db->prepare("INSERT INTO `city` (`city`,`status`) VALUES(?,?)");
            $resa->execute(array($city,$status));
 

  $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><h4><i class="icon fa fa-check"></i>Successfully Inserted</h4></div>';
        } else {
            $res = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><h4><i class="icon fa fa-close"></i>This City is already exists.</h4></div>';
        }
    } else {
        $link1 = FETCH_all("SELECT `id` FROM `city` WHERE `city`=? AND `id`!=?", $city, $getid);
        if ($link1['id'] == '') {
            $resa = $db->prepare("UPDATE `city` SET `city`=?,`status`=? WHERE `id`=?");
            $resa->execute(array($city,$status, $getid));
...
function delcity($a) {
    ...
    foreach ($b as $c) {
        global $db;
          ...
          $get1 = $db->prepare("DELETE FROM `city` WHERE `id` =? ");
         $get1->execute(array(trim($c)));
    }
...
function delmodel($a) {
    ...
    foreach ($b as $c) {
        global $db;
          ...
          $get1 = $db->prepare("DELETE FROM `model` WHERE `id` =? ");
         $get1->execute(array(trim($c)));
    }
...
function addmodel($model,$status,$getid)
{
   global $db;
    if ($getid == '') {
        $link2 = FETCH_all("SELECT `id` FROM `model` WHERE `model`=?", $model);
        if ($link2['id'] == '') {
            $resa = $db->prepare("INSERT INTO `model` (`model`,`status`) VALUES(?,?)");
            $resa->execute(array($model,$status));
 

  $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><h4><i class="icon fa fa-check"></i>Successfully Inserted</h4></div>';
        } else {
            $res = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><h4><i class="icon fa fa-close"></i>This Model is already exists.</h4></div>';
        }
    } else {
        $link1 = FETCH_all("SELECT `id` FROM `model` WHERE `model`=? AND `id`!=?", $model, $getid);
        if ($link1['id'] == '') {
            $resa = $db->prepare("UPDATE `model` SET `model`=?,`status`=? WHERE `id`=?");
            $resa->execute(array($model,$status, $getid));
...
function addsalesman($name,$mobileno,$address,$vehicle_no,$model,$image,$status,$getid) {
    global $db;


    if ($getid == '') {
        $link1 = FETCH_all("SELECT `id` FROM `salesman` WHERE `mobileno`=?", $mobileno);
if($link1['id']=='') {
$resa = $db->prepare("INSERT INTO `salesman` (`name`,`mobileno`,`address`,`vehicle_no`,`model`,`image`,`status`) VALUES (?,?,?,?,?,?,?)");
        $resa->execute(array($name,$mobileno,$address,$vehicle_no,$model,$image,$status));
      
              $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><h4><i class="icon fa fa-check"></i>Successfully Inserted</h4></div>';
    }
    else
    {
          $res = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><h4><i class="icon fa fa-close"></i>Mobileno already exists!</h4></div>';
    }
    } else {
 $link1 = FETCH_all("SELECT `id` FROM `salesman` WHERE `mobileno`=? AND `id`!=?", $mobileno,$getid);
if($link1['id']=='') {
    $resa = $db->prepare("UPDATE `salesman` SET `name`=?,`mobileno`=?,`address`=?,`vehicle_no`=?,`model`=?,`image`=?,`status`=? WHERE `id`=?");
        $resa->execute(array($name,$mobileno,$address,$vehicle_no,$model,$image,$status,$getid));
...
function addobject($barcodeText,$barcodeType,$barcodeDisplay,$barcodeSize,$printText,$barcode_type,$mrp_price,$unit_per_pack,$barcode,$in_stock,$hsn,$productcode,$gst,$objectname,$category,$subcategory,$unit,$price,$status, $getid) 
{
    global $db;
    if ($getid == '') {
        $link2 = FETCH_all("SELECT `id` FROM `object` WHERE `productcode`=? AND `objectname`=? AND `category`=? ", $productcode,$objectname, $category);
        if ($link2['id'] == '') {
            $resa = $db->prepare("INSERT INTO `object` (`barcodeText`,`barcodeType`,`barcodeDisplay`,`barcodeSize`,`printText`,`barcode_type`,`mrp_price`,`unit_per_pack`,`barcode`,`in_stock`,`hsn`,`gst`,`productcode`,`objectname`,`category`,`subcategory`,`unit`,`price`,`status`) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $resa->execute(array($barcodeText,$barcodeType,$barcodeDisplay,$barcodeSize,$printText,$barcode_type,$mrp_price,$unit_per_pack,$barcode,$in_stock,$hsn,$gst,$productcode,$objectname,$category,$subcategory,$unit,$price,$status));
 $l_insert = $db->lastinsertid();
 ...
 $url=$sitename.$l_insert.'/editobject.htm';
        ...
        if ($link1['id'] == '') {
            ...
            $resa = $db->prepare("UPDATE `object` SET `barcodeText`=?,`barcodeType`=?,`barcodeDisplay`=?,`barcodeSize`=?,`printText`=?,`barcode_type`=?,`mrp_price`=?,`unit_per_pack`=?,`barcode`=?,`in_stock`=?,`hsn`=?,`gst`=?,`productcode`=?,`objectname`=?,`category`=?,`unit`=?,`price`=?, `status`=? WHERE `id`=?");
            $resa->execute(array($barcodeText,$barcodeType,$barcodeDisplay,$barcodeSize,$printText,$barcode_type,$mrp_price,$unit_per_pack,$barcode,$in_stock,$hsn,$gst,$productcode,$objectname, $category,$unit,$price,$status, $getid));
...
function addpurchasereturn($mrp_price1,$hsn1,$oid1,$bill_no,$supplierid, $date, $netweight, $amount, $status,$object1, $quantity1,$rate1,$tamount1, $sub_total,$discount,$tot_amt,$payment_type,$advance_amount,$balance_amount,$getid)
{
    global $db;
     if ($getid == '') {
...
//        }
 ...
 $proid=$getproduct['id'];
                    $resa = $db->prepare("INSERT INTO `purchasereturn_object_detail` (`mrp_price`,`hsn`,`productid`,`object`,`pquantity`,`rate`,`amount`,`object_id`,`status`) VALUES (?,?,?,?,?,?,?,?,?) ");
                    $resa->execute(array($mrp_price[$key],$hsn[$key],$proid,$objectval[$key], $quantity[$key],$rate[$key],$tamount[$key], $l_insert, '1'));
...
//             $oid= explode(',',$oid1);
 ...
 $proid=$getproduct['id'];
                    if($oid[$key]=='') { 
                    $resa = $db->prepare("INSERT INTO `purchasereturn_object_detail` (`mrp_price`,`hsn`,`productid`,`object`,`pquantity`,`rate`,`amount`,`object_id`,`status`) VALUES (?,?,?,?,?,?,?,?,?) ");
                    $resa->execute(array($mrp_price[$key],$hsn[$key],$proid,$objectval[$key], $quantity[$key],$rate[$key],$tamount[$key], $getid, '1'));
                    }
                    else {
                         $resa = $db->prepare("UPDATE `purchasereturn_object_detail` SET `mrp_price`=?,`hsn`=?,`productid`=?,`object`=?,`pquantity`=?,`rate`=?,`amount`=?,`object_id`=?,`status`=? WHERE `id`=?");
                    $resa->execute(array($mrp_price[$key],$hsn[$key],$proid,$objectval[$key], $quantity[$key],$rate[$key],$tamount[$key], $getid, '1',$oid[$key]));
                    }
...
function addpurchaseorder($mrp_price1,$hsn1,$oid1,$bill_no,$supplierid, $date, $netweight, $amount, $status,$object1, $quantity1,$rate1,$tamount1, $sub_total,$discount,$tot_amt,$payment_type,$advance_amount,$balance_amount,$getid)
{
    global $db;
     if ($getid == '') {
...
$cresa->execute(array('purchase',$bill_no,$supplierid,$l_insert,$tot_amt,$advance_amount,$balance_amount));
 ...
 $proid=$getproduct['id'];
                    $resa = $db->prepare("INSERT INTO `purchaseorder_object_detail` (`mrp_price`,`hsn`,`productid`,`object`,`pquantity`,`rate`,`amount`,`object_id`,`status`) VALUES (?,?,?,?,?,?,?,?,?) ");
                    $resa->execute(array($mrp_price[$key],$hsn[$key],$proid,$objectval[$key], $quantity[$key],$rate[$key],$tamount[$key], $l_insert, '1'));
...
$cresa->execute(array($bill_no,$supplierid,$tot_amt,$advance_amount,$balance_amount,$getid,'purchase')); 
 ...
 $proid=$getproduct['id'];
                    ...
                    $resa->execute(array($mrp_price[$key],$hsn[$key],$proid,$objectval[$key], $quantity[$key],$rate[$key],$tamount[$key], $getid, '1'));
                         ...
                         $resa = $db->prepare("UPDATE `purchaseorder_object_detail` SET `mrp_price`=?,`hsn`=?,`productid`=?,`object`=?,`pquantity`=?,`rate`=?,`amount`=?,`object_id`=?,`status`=? WHERE `id`=?");
                    $resa->execute(array($mrp_price[$key],$hsn[$key],$proid,$objectval[$key], $quantity[$key],$rate[$key],$tamount[$key], $getid, '1',$oid[$key]));
                    }
...
function addpurchase($mrp_price1,$hsn1,$oid1,$bill_no,$supplierid, $date, $netweight, $amount, $status,$object1, $quantity1,$rate1,$tamount1, $sub_total,$discount,$tot_amt,$payment_type,$advance_amount,$balance_amount,$getid)
{
    global $db;
     if ($getid == '') {
...
$cresa->execute(array('purchase',$bill_no,$supplierid,$l_insert,$tot_amt,$advance_amount,$balance_amount));
 ...
 $proid=$getproduct['id'];
                    $resa = $db->prepare("INSERT INTO `purchase_object_detail` (`mrp_price`,`hsn`,`productid`,`object`,`pquantity`,`rate`,`amount`,`object_id`,`status`) VALUES (?,?,?,?,?,?,?,?,?) ");
                    $resa->execute(array($mrp_price[$key],$hsn[$key],$proid,$objectval[$key], $quantity[$key],$rate[$key],$tamount[$key], $l_insert, '1'));
...
$cresa->execute(array($bill_no,$supplierid,$tot_amt,$advance_amount,$balance_amount,$getid,'purchase')); 
 ...
 $proid=$getproduct['id'];
                    ...
                    $resa->execute(array($mrp_price[$key],$hsn[$key],$proid,$objectval[$key], $quantity[$key],$rate[$key],$tamount[$key], $getid, '1'));
						 ...
						 $resa = $db->prepare("UPDATE `purchase_object_detail` SET `mrp_price`=?,`hsn`=?,`productid`=?,`object`=?,`pquantity`=?,`rate`=?,`amount`=?,`object_id`=?,`status`=? WHERE `id`=?");
                    $resa->execute(array($mrp_price[$key],$hsn[$key],$proid,$objectval[$key], $quantity[$key],$rate[$key],$tamount[$key], $getid, '1',$oid[$key]));
					}
...
$resa = $db->prepare("INSERT INTO `online_order` (`salesman`,`billtypemast`,`bill_advanceamt`,`bill_balanceamt`,`gsttype`,`gstvalue`,`createdby`,`created_id`,`billtype`,`order_method`,`payment_mode`,`customer_id`,`order_type`,`date`,`bill_number`,`given_amnt`,`balance_amnt`,`sub_tot`,`discount`,`packing_charges`,`delivery_charges`,`total_amnt`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

$resa->execute(array($salesman,$billtypemast,$bill_advanceamt,$bill_balanceamt,$gsttype,$gstvalue,$createdby,$createdid,$billtype,$order_method,$payment_mode,$customer_name,$order_type,$date,$bill_no,$given_amt,$balance_amt,$sub_total, $discount, $packing_charge, $delivery_charge, $tot_amt));
            ...
            foreach ($objectval as $key => $value)
             {
                ...
                if ($objectval[$key] != '') 
                {
                    ...
                    $resa = $db->prepare("INSERT INTO `online_order_deatils` (`hsn`,`gstresult`,`object_id`,`bill_no`,`product_name`,`qty`,`rate`,`total`,`mrp_price`) VALUES (?,?,?,?,?,?,?,?,?) ");
            $resa->execute(array($hsn[$key],$gstresult[$key],$l_insert,$bill_no,$objectval[$key], $quantity[$key], $rate[$key], $total[$key], $mrp_price[$key]));
...
$resa = $db->prepare("UPDATE `online_order` SET `salesman`=?,`billtypemast`=?,`bill_advanceamt`=?,`bill_balanceamt`=?,`gsttype`=?,`gstvalue`=?,`billtype`=?,`order_method`=?,`payment_mode`=?,`customer_id`=?,`order_type`=?,`date`=?,`bill_number`=?,`given_amnt`=?,`balance_amnt`=?,`sub_tot`=?,`discount`=?,`packing_charges`=?,`delivery_charges`=?,`total_amnt`=? WHERE `id`=?");

$resa->execute(array($salesman,$billtypemast,$bill_advanceamt,$bill_balanceamt,$gsttype,$gstvalue,$billtype,$order_method,$payment_mode,$customer_name,$order_type,$date,$bill_no,$given_amt,$balance_amt,$sub_total, $discount, $packing_charge, $delivery_charge, $tot_amt,$getid));
            ...
            foreach ($objectval as $key => $value)
             {
                ...
                {
                    if($oid[$key]=='') {
                    $resa = $db->prepare("INSERT INTO `online_order_deatils` (`mrp_price`,`hsn`,`gstresult`,`object_id`,`bill_no`,`product_name`,`qty`,`rate`,`total`) VALUES (?,?,?,?,?,?,?,?,?) ");
            $resa->execute(array($mrp_price[$key],$hsn[$key],$gstresult[$key],$getid,$bill_no,$objectval[$key], $quantity[$key], $rate[$key], $total[$key]));
					}
					else
					{
					 $resa = $db->prepare("UPDATE `online_order_deatils` SET `mrp_price`=?,`hsn`=?,`gstresult`=?,`object_id`=?,`bill_no`=?,`product_name`=?,`qty`=?,`rate`=?,`total`=? WHERE `id`=? ");
            $resa->execute(array($mrp_price[$key],$hsn[$key],$gstresult[$key],$getid,$bill_no,$objectval[$key], $quantity[$key], $rate[$key], $total[$key], $oid[$key]));	
					}
