...
$als = FETCH_all("SELECT * FROM `sales_return` WHERE `bill_number`=?", $salesorder_billno);
if($als['id']!='') {
$link22 = FETCH_all("SELECT * FROM `sales_return` WHERE `id`=?", $als['id']); 
$lid=$link22['id'];  
}
else
{
$link22 = FETCH_all("SELECT * FROM `sales_order` WHERE `bill_number`=?", $salesorder_billno);    
}
...
if ($_REQUEST['lid'] != '') {
$lid=$_REQUEST['lid'];
 $link22 = FETCH_all("SELECT * FROM `sales_return` WHERE `id`=?", $lid);   
 }


$exerecord = FETCH_all("SELECT * FROM `sales_return` WHERE `bill_number`=?", $link22['bill_number']);   
...
$balcamt=$gtotal-$link22['bill_advanceamt'];    
       ...
       echo '<script>window.location.href="' . $sitename . 'pages/process/addsales_return.php?delsttaus=temp&salesorder_billno=' . $_REQUEST['salesorder_billno'] . '"</script>';   
...
$url="addsales_return.php?pid=".$insid; 
...
if (isset($_REQUEST['delid1']) && $_REQUEST['delid1'] != '') {
    $up = $db->prepare("DELETE FROM `sales_return_details` WHERE `id`=?");
    $up->execute(array($_REQUEST['delid1']));
    $a = $_REQUEST['lid'];
   if($a!='') { 
     $lids=$_REQUEST['lid'];
      $tobjects = $db->prepare("SELECT SUM(`total`) AS `totsub` FROM `sales_return_details` WHERE `object_id`= ?");
...
$balcamt=$gtotal-$link22['bill_advanceamt'];    
    ...
    echo '<script>window.location.href="' . $sitename . 'process/' + $_REQUEST['lid'] + '/editsales_return.htm"</script>';
    }else{
       echo '<script>window.location.href="' . $sitename . 'process/addsales_return.htm"</script>';   
    }
}
?>
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="<?php echo $sitename; ?>pages/process/sales_return.php"><i class="fa fa-circle-o"></i> Sales</a></li>
            <!-- <li class="active"><?php
                if ($_REQUEST['lid'] != '') {
                    echo 'Edit';
                } else {
...
{
?>
 <option value="<?php echo $customerfetch['id']; ?>" <?php if($link22['supplierid']==$customerfetch['id'] || $customerfetch['id']==$_REQUEST['pid']) { ?> selected="selected" <?php } ?>><?php echo $customerfetch['name']; ?></option>
<?php } ?>                          
                             </select>
...
<div class="col-md-6">
    <table width="100%">
                        ...
                        <tr>
                        <td colspan="2"><input type="text" class="form-control" readonly="readonly" name="bill_no" id="bill_no" style="width:fit-content;"  value="<?php if($_REQUEST['salesorder_billno']!='') { echo $_REQUEST['salesorder_billno']; } else { echo $link22['bill_no']; } ?>">
                        </td></tr>
                        </table>
...
$object1 = $db->prepare("SELECT * FROM `temp_sales_details` WHERE `object_id`= ?");
                                                ...
                                                $object1 = $db->prepare("SELECT * FROM `sales_return_details` WHERE `object_id`= ?");
                                                }
                                                else {
        ...
        // $get->execute(array());
                                                 ...
                                                 $objectsss1 = $db->prepare("SELECT * FROM `sales_order_details` WHERE `object_id`= ?"); 
...
if($getrecr['id']==''){
                                                    ...
                                                    $qty=$objectsss1list['qty'];
                                                     ...
                                                     $resa = $db->prepare("INSERT INTO `temp_sales_details` (`hsn`,`gstresult`,`object_id`,`bill_no`,`product_name`,`qty`,`rate`,`total`) VALUES (?,?,?,?,?,?,?,?) ");
            $resa->execute(array($hsn,$gstresult,$object_id,$bill_no,$product_name,$qty,$rate,$total));
}
...
<td><input type="text" style="text-align: right;" name="rate[]" id="rate[]" readonly="readonly" class="form-control rate" value="" /><input type="hidden" name="ratehidden" id="ratehidden" value="" onkeyup="quantitycalculation(this.value)"/></td>
...
<td><input type="text" style="text-align: right;" name="mrp_price[]"  readonly="readonly" id="mrp_price" class="form-control mrp_price" /></td>
...
<?php } ?>
    <option value="Cash Payment" <?php if(trim($link22['payment_mode']=='Cash Payment')) { ?> selected="selected" <?php } ?>>Cash Payment</option>
    <option value="Card Payment" <?php if(trim($link22['payment_mode'])=='Card Payment') { ?> selected="selected" <?php } ?>>Card Payment</option>
    <option value="UPI Payment" <?php if(trim($link22['payment_mode'])=='UPI Payment') { ?> selected="selected" <?php } ?>>Google Pay</option>
    </select>
...
<table border="1" cellpadding="10" style="width:372px;font-size:13px; font-weight:bold;" align="right">
  ...
  <tr>
    ...
    <div class="col-md-6" style="padding-right: 0px;">GST Value:</div>
       <div class="col-md-6"  style="padding-left: 20px;"><input class="form-control"  type="text" <?php if($_REQUEST['type']=='view') { ?>  readonly="readonly" <?php } ?> name="gstvalue" id="gstvalue" value="<?php if($gstvalue!='') { echo $gstvalue; } else { echo stripslashes($link22['gstvalue']); } ?>"></div>
</div>
   </td>
...
$('#tot_amt').val(gtotal);
    ...
    function delrec1(elem, id,lid) {
        if (confirm("Are you sure want to delete this Details?")) {
            $(elem).parent().remove();
 <?php if($_REQUEST['lid']!='') { ?>
            window.location.href = "<?php echo $sitename; ?>process/<?php echo $_REQUEST['lid']; ?>/editsales_return.htm?delid1=" + id;
            <?php } elseif($lid!='') { ?>
            window.location.href = "<?php echo $sitename; ?>process/<?php echo $lid; ?>/editsales_return.htm?delid1=" + id;
            <?php }  else { ?>
              window.location.href = "<?php echo $sitename; ?>process/addsales_return.htm?delid1=" + id;
            <?php } ?>
        }
    }
...
function getordertype(a) {
     ...
     function delrecss1(elem, id,billno) {
        if (confirm("Are you sure want to delete this Details?")) {
            $(elem).parent().remove();
              window.location.href = "<?php echo $sitename; ?>pages/process/addsales_return.php?delsttaus=temp&delidss1=" + id + "&salesorder_billno="+ billno;
