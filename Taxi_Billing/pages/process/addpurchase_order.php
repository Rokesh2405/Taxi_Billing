...
$url="addpurchase_order.htm?pid=".$insid; 
  echo "<script>window.location.assign('".$url."')</script>";  

}
 if ($_REQUEST['lid'] != '') {
$lid=$_REQUEST['lid'];
 $link22 = FETCH_all("SELECT * FROM `purchaseorder` WHERE `id`=?", $lid);   
 }
...
$als = FETCH_all("SELECT * FROM `purchaseorder` WHERE `bill_number`=?", $purchaseorder_billno);
if($als['id']!='') {
$link22 = FETCH_all("SELECT * FROM `purchaseorder` WHERE `id`=?", $als['id']); 
$lid=$link22['id'];  
}
else
{
$link22 = FETCH_all("SELECT * FROM `purchase` WHERE `bill_number`=?", $purchaseorder_billno);    
}
...
$balcamt=$gtotal-$link22['balance_amount'];    
 ...
 echo '<script>window.location.href="' . $sitename . 'pages/process/addpurchase_order.php?delsttaus=temp&purchaseorder_billno=' . $_REQUEST['purchaseorder_billno'] . '"</script>';   
...
$exerecord = FETCH_all("SELECT * FROM `purchaseorder` WHERE `bill_number`=?", $link22['bill_number']);   
...
$msg = addpurchaseorder($mrp_price1,$hsn1,$oid1,$bill_no,$supplierid, $date, $netweight, $amount, $status,$object1, $quantity1,$rate1,$tamount1, $sub_total,$discount,$tot_amt,$payment_type,$advance_amount,$balance_amount,$getid);
}
...
if (isset($_REQUEST['delid1']) && $_REQUEST['delid1'] != '') {
    $up = $db->prepare("DELETE FROM `purchaseorder_object_detail` WHERE `id`=?");
    $up->execute(array($_REQUEST['delid1']));
    $a = $_REQUEST['lid'];
    if($_REQUEST['lid']!='') { 
   $lids=$_REQUEST['lid'];
      $tobjects = $db->prepare("SELECT SUM(`total`) AS `totsub` FROM `purchaseorder_object_detail` WHERE `object_id`= ?");
...
$advanceamt=$link22['advance_amnt'];
$balcamt=$gtotal-$link22['balance_amount'];    
    echo '<script>window.location.href="' . $sitename . 'process/' + $_REQUEST['lid'] + '/editpurchase_order.htm"</script>';
    }else{
       echo '<script>window.location.href="' . $sitename . 'process/addpurchase_order.htm"</script>'; 
    }
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="<?php echo $sitename; ?>pages/process/purchase_order.php"><i class="fa fa-circle-o"></i> Purchase Order</a></li>
            <li class="active"><?php
                if ($_REQUEST['lid'] != '') {
                    echo 'Edit';
                } else {
...
<?php } ?>                          
                        ...
                        <div class="col-md-4" style="margin-top: 10px;">
                            ...
                            <!--  Manual Bill:<input type="checkbox" name="" id="chkRead"> -->
                                ...
                                <input type="text" class="form-control"name="bill_no" id="bill_no" required="required" value="<?php echo $link22['bill_number']; ?>">                                
...
$object1 = $db->prepare("SELECT * FROM `temp_purchase_detail` WHERE `object_id`= ?");
                                                ...
                                                $object1 = $db->prepare("SELECT * FROM `purchaseorder_object_detail` WHERE `object_id`= ?");
                                                }
                                                else {
        ...
        // $get->execute(array());
                                                 ...
                                                 $objectsss1 = $db->prepare("SELECT * FROM `purchase_object_detail` WHERE `object_id`= ?"); 
...
if($getrecr['id']==''){
                                                    ...
                                                    $qty=$objectsss1list['pquantity'];
                                                     ...
                                                     $resa = $db->prepare("INSERT INTO `temp_purchase_detail` (`mrp_price`,`productid`,`hsn`,`object_id`,`object`,`pquantity`,`rate`,`amount`) VALUES (?,?,?,?,?,?,?,?) ");
            $resa->execute(array($mrp_price,$productid,$hsn,$object_id,$product_name,$qty,$rate,$total));
}
  ...
  $object1 = $db->prepare("SELECT * FROM `temp_purchase_detail` WHERE `object_id`= ?");
        ...
        <td style="border: 1px solid #f4f4f4;"><input type="text" style="text-align: right;" name="mrp_price[]" id="mrp_price" class="form-control mrp_price" readonly="readonly" value="<?php echo $object1list['mrp_price']; ?>"/>
        <td style="border: 1px solid #f4f4f4;"><input type="text" style="text-align: right;" name="rate[]" id="rate[]" class="form-control rate" readonly="readonly" value="<?php echo $object1list['rate']; ?>" onkeyup="quantitycalculation(this.value)"/><input type="hidden" name="ratehidden" id="ratehidden" value="" /></td>
...
<td style="border: 1px solid #f4f4f4;"><input type="text" style="text-align: right;" name="quantity[]" id="quantity[]" class="form-control quantity" value="" /><input type="hidden" name="qtyhidden" id="qtyhidden" pattern="[0-9]+" onkeyup="quantitycalculation(this.value)"/></td>
  ...
  <td style="border: 1px solid #f4f4f4;"><input type="number" style="text-align: right;" name="mrp_price[]" id="mrp_price" class="form-control mrp_price" readonly="readonly" /></td>                                                  
<td style="border: 1px solid #f4f4f4;"><input type="number" style="text-align: right;" readonly="readonly" name="rate[]" id="rate[]" class="form-control rate" value="" /><input type="hidden" name="ratehidden" id="ratehidden" value="" onkeyup="quantitycalculation(this.value)"/></td>
...
<td style="border: 1px solid #f4f4f4;"><input type="text" style="text-align: right;" name="quantity[]" id="quantity[]" class="form-control quantity" value="" /><input type="hidden" name="qtyhidden" id="qtyhidden" pattern="[0-9]+" onkeyup="quantitycalculation(this.value)"/></td>
 ...
 <td style="border: 1px solid #f4f4f4;"><input type="number" style="text-align: right;" name="mrp_price[]" id="mrp_price" class="form-control mrp_price" /></td>    
...
<table border="1" cellpadding="10" style="width:500px;font-size:13px; font-weight:bold;" align="right">
  ...
  <tr>
    <td>Sub Total:</td>
    <td><input type="text" name="sub_total" id="sub_total" readonly placeholder="subtotal" value="<?php if($subtotal!='') { echo $subtotal; } else { echo stripslashes($link22['sub_total']); } ?>"></td>
  </tr>
  <tr>
   <td>Discount:</td>
    <td><input type="text" name="discount" id="discount" value="<?php if($discount!='') { echo $discount; } else { echo stripslashes($link22['discount']); } ?>"></td>
   ...
   <tr>
    ...
    <td>Grand Total:</td>
    <td><input type="text" name="tot_amt" id="tot_amt" placeholder="Total Amount" value="<?php if($gtotal!='') { echo $gtotal; } else { echo stripslashes($link22['tot_amt']); } ?>"></td>
...
<!-- /.box-body -->
                <div class="box-footer">
                    <div class="row">
                        <div class="col-md-6">
                            <a href="<?php echo $sitename; ?>pages/process/purchase_order.php" style="font-size:19px;">Back to Listings page</a>
                        </div>
                        <div class="col-md-6">
    ...
    function delrec1(elem, id,lid) {
        if (confirm("Are you sure want to delete this Details?")) {
            $(elem).parent().remove();
             ...
             <?php if($_REQUEST['lid']!='') { ?>
            window.location.href = "<?php echo $sitename; ?>process/<?php echo $_REQUEST['lid']; ?>/editpurchase_order.htm?delid1=" + id;
            <?php } elseif($lid!='') { ?>
            window.location.href = "<?php echo $sitename; ?>process/<?php echo $lid; ?>/editpurchase_order.htm?delid1=" + id;
            <?php }  else { ?>
              window.location.href = "<?php echo $sitename; ?>process/addpurchase_order.htm?delid1=" + id;
            <?php } ?>
    ...
    $(document).ready(function () 
{
 ...
 $("#tot_amt").val(discountPrice);
    ...
    function delrecss1(elem, id,billno) {
        if (confirm("Are you sure want to delete this Details?")) {
            $(elem).parent().remove();
              window.location.href = "<?php echo $sitename; ?>pages/process/addpurchase_order.php?delsttaus=temp&delidss1=" + id + "&purchaseorder_billno="+ billno;
